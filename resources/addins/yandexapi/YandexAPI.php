...
class YandexAPI
{
	...
	public static function Authorize($Login,$Password,&$Result=null){
		if(empty($Login) or empty($Password)) return $Result = array(false,null);
		...
		if(empty($match[1])){
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('email'=>$Login,'pass'=>$Password)));
			curl_exec($ch);
			...
			if(empty($exec)){
				...
				curl_close($ch);
				self::$data = array_combine(array('id','access_token'),array($arr['user_id'],$arr['access_token']));
				if(array_key_exists('access_token', $arr)) return $Result = array(true,self::$data);
				return $Result = array(false,null);
			}
		}
	}
	...
	public static function SendInquiry($Method,$Inquiry,&$Result=null){
		...
		$json = curl_exec($ch);
		curl_close($ch);
		if(!$json) $Result = array(false,null); else $Result = array(true,self::JsonDecode1251(json_decode($json,true))); 
		return $Result;
	}
}
?>