...
class evfmEasySelectDialog {
    ...
    static function onActivate($self){
        ...
        DevS\cache::c('objs_forms')->items->setArray(array_keys($arr));
        DevS\cache::c('objs_forms')->itemIndex = $formSelected;
        
        DevS\cache::c('lst_forms')->items->setArray(array_keys($arr));
        DevS\cache::c('lst_forms')->itemIndex = $formSelected;
        
        myInspect::generateEx($arr[ $_FORMS[$formSelected] ], DevS\cache::c('objs_list'));
        myInspect::generateEx($arr[ $_FORMS[$formSelected] ], DevS\cache::c('lst_objects'));
        
        $txt = DevS\cache::c('line')->text;
        DevS\cache::c('c_kav')->checked = ($txt[0]=='"' && $txt[strlen($txt)-1]=='"');
        if ( preg_match('/^c\((.*)\)$/i', trim($txt)) ){
            ...
            $objs = explode('->',$obj_str);
                ...
                if (in_array($objs[0],$_FORMS)){
                    DevS\cache::c('objs_forms')->itemIndex = array_search($objs[0], $_FORMS);
                    myInspect::generateEx($arr[ $objs[0] ], DevS\cache::c('objs_list'));
                    DevS\cache::c('objs_forms')->setFocus();
                    if ( $objs[1] ){
                        DevS\cache::c('objs_list')->items->selectedCaption = $objs[1];
                        DevS\cache::c('objs_list')->setFocus();
                    }
                } else {
                    DevS\cache::c('objs_list')->items->selectedCaption = $objs[0];
                    DevS\cache::c('objs_list')->setFocus();
                }
            ...
            if ( $obj_str ){
                ...
                DevS\cache::c('lst_objects')->items->selectedCaption = $obj_str;
            }
            
            ev_lst_objects::onClick(DevS\cache::c('lst_objects')->self);
            DevS\cache::c('lst_props')->setFocus();
                
            DevS\cache::c('lst_props')->items->selected = myProperties::getPropertyText($prop);
            DevS\cache::c('lst_props')->setFocus();
            DevS\cache::c('line')->text = $arx[0][0];
            ...
            if ( !$objs[1] )
                DevS\cache::c('lst_objects')->itemIndex = -1;
        }
    }
    ...
    static function onColorClick($self){
       DevS\cache::c('line')->text = _c($self)->text;
    }
    
    static function onColorSelect($obj, $color){
        DevS\cache::c('line')->text = '0x'.dechex($color);
    }
}
...
class ev_objs_forms {
    ...
    static function onChange($self){
        ...
        if ($GLOBALS['OBJ_ISFUNC'])
            DevS\cache::c('line')->text = _c($self)->items->selected;
        else
            DevS\cache::c('line')->text = 'c(\'' . _c($self)->items->selected . '\')';
    }
...
class ev_lst_forms {
    ...
    static function onChange($self){
        ...
        DevS\cache::c('lst_objects')->items->itemIndex = -1;
...
class ev_objs_list {
    ...
    static function onClick($self){
        ...
        $form_name = DevS\cache::c('objs_forms')->items->selected;
        
        if ($GLOBALS['OBJ_ISFUNC']){
            if (current($objs))
                if ($GLOBALS['OBJ_FULLPATH'])
                    DevS\cache::c('line')->text = $form_name .'->'. current($objs);
                else
                    DevS\cache::c('line')->text = current($objs);
            else
                DevS\cache::c('line')->text = $form_name;
        } else {
            if (current($objs)){
                if ($_FORMS[$formSelected]==$form_name)
                    DevS\cache::c('line')->text = 'c("' . current($objs) . '")';
                else
                    DevS\cache::c('line')->text = 'c("' . $form_name.'->'. current($objs) . '")';
            }
            else
                DevS\cache::c('line')->text = 'c("' . $form_name . '")';
        }
    }
...
class ev_lst_objects {
    ...
    static function onClick($self){
        ...
        $findex = DevS\cache::c('lst_forms')->items->selected;
        ...
        DevS\cache::c('lst_props')->items->setArray($result);
        $form_name = DevS\cache::c('lst_forms')->items->selected;
        
        if ($class!='TForm'){
            
            if ($_FORMS[$formSelected]==$form_name)
               DevS\cache::c('line')->text = 'c("' . current($obj->items->selectedCaption) . '")';
            else
                DevS\cache::c('line')->text = 'c("' . $form_name .'->'. current($obj->items->selectedCaption) . '")';
        }
        else
            DevS\cache::c('line')->text = 'c("' . $form_name . '")';
    }
...
class ev_lst_props {
    ...
    static function onClick($self){
        ...
        DevS\cache::c('line')->text = $result . '->' . $prop;
    }
}
...
class ev_fd_browsedir {
    ...
    static function onClick($self){
        ...
        if (selectDirectory('','',$dir)){
            ...
            DevS\cache::c('line')->text  = $dir;
            DevS\cache::c('l_dir')->text = $dir;
        }
    }
}
...
class ev_fd_browsefiles {
    ...
    static function onClick($self){
        ...
        if ($dlg->execute()){
            ...
            DevS\cache::c('line')->text   = $file;
            DevS\cache::c('l_file')->text = $file;
        }
...
class ev_fd_browsedirlocal {
    ...
    static function onClick($self){
        ...
        if (selectDirectory('',dirname($projectFile),$dir)){
            ...
            DevS\cache::c('line')->text  = $dir;
            DevS\cache::c('l_dirlocal')->text = $dir;
        }
    }
}
...
class ev_fd_browsefileslocal {
    ...
    static function onClick($self){
        ...
        if ($dlg->execute()){
            ...
            DevS\cache::c('line')->text   = $file;
            DevS\cache::c('l_filelocal')->text = $file;
        }
...
class ev_c_kav {
    ...
    static function onMouseUp($self){
        ...
        if (c($self)->checked){
            DevS\cache::c('line')->text = '"' . c('line')->text . '"';
        } else {
            DevS\cache::c('line')->text = action_Simple::trimQuote(c('line')->text);
        }
    }
}